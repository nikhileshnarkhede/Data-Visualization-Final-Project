...
// Add "All" button to show all lines
buttonsContainer.append("button")
    ...
    .style("border-radius", "5px")
    .style("background-color", "#aaa") // Default color for the "Show All" button
    .on("click", function () {
...
categories.forEach(category => {
    buttonsContainer.append("button")
        .attr("class", "button-" + category.replace(/\s+/g, "-"))
        .text(category)
        .style("padding", "10px")
        .style("width", "250px")
        .style("font-size", "14px")
        .style("cursor", "pointer")
        .style("border-radius", "5px")
        .style("background-color", color(category)) // Set button color based on the line color
        .on("click", function () {
            ...
            linesMap.forEach((line, categoryKey) => {
                if (categoryKey === category) {
                    line.transition()
                        .duration(300)
                        .style("opacity", 1); // Show the selected line
                } else {
                    line.transition()
                        .duration(300)
                        .style("opacity", 0); // Hide all other lines
                }
            });
        });
});
